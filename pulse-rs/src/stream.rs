...
impl Stream {
    ...
    pub fn peek(&self) -> Result<(*const c_void, usize)> {
        let mut data: *const c_void = ptr::null();
        let mut length: usize = 0;
        let r = unsafe { ffi::pa_stream_peek(self.raw_mut(), &mut data, &mut length) };
        ...
        where CB: Fn(&Stream, i32, *mut c_void)
    {
        ...
        unsafe extern "C" fn wrapped<F>(s: *mut ffi::pa_stream,
                                        success: c_int,
                                        userdata: *mut c_void)
